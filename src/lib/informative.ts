...
export async function resolveInformativeGuideline(id: string) {
  ...
  const informativeGuideline = await silenceWarnings(() => getEntry("howtoGuidelines", id));
...
export async function resolveInformativeRequirement(id: string) {
  ...
  const informativeRequirement = await silenceWarnings(() => getEntry("howtoRequirements", id));
...
export async function resolveInformativeGroups() {
  ...
  const populatedGroupIds = (await getCollection("howtoGuidelines")).reduce((set, entry) => {
